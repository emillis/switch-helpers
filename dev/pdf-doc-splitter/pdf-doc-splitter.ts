...
export class Splitter {
    ...
    private docsToSave: {range: string, pdf: pdfLib.PDFDocument}[] = [];
    ...
    async split(split: (string | number)[]) {
        this.wasInitialized()
        if (!this.docToSplit) throw `Document is not assigned!`;

        if (!split.length) split = this.docToSplit.getPageIndices().map(v=>{return v+1})
